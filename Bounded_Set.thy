...
declare Bounded_Set.bset.rel_mono[mono]
...
lemma inj_map_bset:
  assumes "inj f" shows "inj (map_bset f)"
by (rule injI) (metis assms inj_image_eq_iff map_bset.rep_eq set_bset_inverse)

lemma rel_bset_symp: "symp R \<Longrightarrow> symp (rel_bset R)"
by (auto simp add: symp_def rel_bset_def rel_set_def) metis+

lemma rel_bset_transp: "transp R \<Longrightarrow> transp (rel_bset R)"
