...
section \<open>Infinitary Formulas With Effects\<close>
...
next
  ...
  show "(p \<bullet> t, p \<bullet> tAct f \<alpha> t) \<in> Tree_wf"
...
lemma supp_tPred [simp]: "supp (tPred f \<phi>) = supp f \<union> supp \<phi>"
unfolding supp_def by (simp add: Collect_imp_eq Collect_neg_eq)

lemma supp_tAct [simp]: "supp (tAct f \<alpha> t) = supp f \<union> supp \<alpha> \<union> supp t"
...
quotient_type
  ('idx,'pred,'act,'eff) Tree\<^sub>p = "('idx,'pred::pt,'act::bn,'eff::fs) Tree" / "hull_relp"
...
lift_definition Tree_wf\<^sub>p :: "('idx,'pred::pt,'act::bn,'eff::fs) Tree\<^sub>p rel" is
  Tree_wf .
...
using assms by (metis (erased, lifting) Pair_inject Tree_wf\<^sub>p.abs_eq prod_fun_imageE) (* DIFF *)
...
fun alpha_Tree_termination :: "('a, 'b, 'c, 'd) Tree \<times> ('a, 'b, 'c, 'd) Tree \<Rightarrow> ('a, 'b::pt, 'c::bn, 'd::fs) Tree\<^sub>p set" where
...
next
  ...
  {
    fix a b
    have "tAct f1 \<alpha>1 t1 =\<^sub>\<alpha> tAct f2 \<alpha>2 t2"
      using tAct.hyps by simp
    then have "(a \<rightleftharpoons> b) \<bullet> tAct f1 \<alpha>1 t1 =\<^sub>\<alpha> tAct f1 \<alpha>1 t1 \<longleftrightarrow> (a \<rightleftharpoons> b) \<bullet> tAct f2 \<alpha>2 t2 =\<^sub>\<alpha> tAct f2 \<alpha>2 t2"
...
proof
  ...
  proof
    ...
    moreover
    {
      ...
      from x1 have "infinite {b. (x \<rightleftharpoons> b) \<bullet> f \<noteq> f}"
        unfolding supp_def ..
      then have "infinite ({b. (x \<rightleftharpoons> b) \<bullet> f \<noteq> f} - supp f)"
        by (simp add: finite_supp)
      moreover
      {
        fix b
        assume "b \<in> {b. (x \<rightleftharpoons> b) \<bullet> f \<noteq> f} - supp f"
        then have b1: "(x \<rightleftharpoons> b) \<bullet> f \<noteq> f" and b2: "b \<notin> supp f"
          by simp+
        then have "sort_of x = sort_of b"
          using swap_different_sorts by fastforce
        then have "(x \<rightleftharpoons> b) \<bullet> supp f \<noteq> supp f"
...
next
  ...
  proof
    ...
    {
      fix b
      assume "\<not> (x \<rightleftharpoons> b) \<bullet> tAct f \<alpha> t =\<^sub>\<alpha> tAct f \<alpha> t"
      then have "(x \<rightleftharpoons> b) \<bullet> f \<noteq> f \<or> (x \<rightleftharpoons> b) \<bullet> \<alpha> \<noteq> \<alpha> \<or> \<not> (x \<rightleftharpoons> b) \<bullet> t =\<^sub>\<alpha> t"
        using alpha_Tree_tAct by force
    }
    ultimately have "infinite {b. (x \<rightleftharpoons> b) \<bullet> f \<noteq> f \<or> (x \<rightleftharpoons> b) \<bullet> \<alpha> \<noteq> \<alpha> \<or> \<not> (x \<rightleftharpoons> b) \<bullet> t =\<^sub>\<alpha> t}"
      by (smt infinite_mono mem_Collect_eq)
    ...
    moreover
    {
      ...
      ultimately have "(x \<rightleftharpoons> b) \<bullet> tAct f \<alpha> t =\<^sub>\<alpha> tAct f \<alpha> t"
        by simp
      with b0 have "False" ..
    }
...
quotient_type
  ('idx,'pred,'act,'eff) Tree\<^sub>\<alpha> = "('idx,'pred::pt,'act::bn,'eff::fs) Tree" / "alpha_Tree"
...
instantiation Tree\<^sub>\<alpha> :: (type, pt, bn, fs) pt
begin

  lift_definition permute_Tree\<^sub>\<alpha> :: "perm \<Rightarrow> ('a,'b,'c,'d) Tree\<^sub>\<alpha> \<Rightarrow> ('a,'b,'c,'d) Tree\<^sub>\<alpha>"
    is permute
  by (fact alpha_Tree_eqvt)

  instance
  proof
    fix t\<^sub>\<alpha> :: "(_,_,_,_) Tree\<^sub>\<alpha>"
    show "0 \<bullet> t\<^sub>\<alpha> = t\<^sub>\<alpha>"
      by transfer (metis alpha_Tree_equivp equivp_reflp permute_zero)
  next
    fix p q :: perm and t\<^sub>\<alpha> :: "(_,_,_,_) Tree\<^sub>\<alpha>"
...
lift_definition Conj\<^sub>\<alpha> :: "('idx,'pred,'act,'eff) Tree\<^sub>\<alpha> set['idx] \<Rightarrow> ('idx,'pred::pt,'act::bn,'eff::fs) Tree\<^sub>\<alpha>" is
  tConj
by simp
...
lift_definition Not\<^sub>\<alpha> :: "('idx,'pred,'act,'eff) Tree\<^sub>\<alpha> \<Rightarrow> ('idx,'pred::pt,'act::bn,'eff::fs) Tree\<^sub>\<alpha>" is
  tNot
by simp

lift_definition Pred\<^sub>\<alpha> :: "'eff \<Rightarrow> 'pred \<Rightarrow> ('idx,'pred::pt,'act::bn,'eff::fs) Tree\<^sub>\<alpha>" is
  tPred
.

lift_definition Act\<^sub>\<alpha> :: "'eff \<Rightarrow> 'act \<Rightarrow> ('idx,'pred,'act,'eff) Tree\<^sub>\<alpha> \<Rightarrow> ('idx,'pred::pt,'act::bn,'eff::fs) Tree\<^sub>\<alpha>" is
  tAct
...
lemma Pred\<^sub>\<alpha>_eqvt [eqvt, simp]: "p \<bullet> Pred\<^sub>\<alpha> f \<phi> = Pred\<^sub>\<alpha> (p \<bullet> f) (p \<bullet> \<phi>)"
by (simp add: Pred\<^sub>\<alpha>.abs_eq)

lemma Act\<^sub>\<alpha>_eqvt [eqvt, simp]: "p \<bullet> Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha> = Act\<^sub>\<alpha> (p \<bullet> f) (p \<bullet> \<alpha>) (p \<bullet> t\<^sub>\<alpha>)"
...
proof
  ...
  then show "f1 = f2 \<and> \<phi>1 = \<phi>2"
    using alpha_Tree.cases by auto
next
  assume "f1 = f2 \<and> \<phi>1 = \<phi>2" then show "Pred\<^sub>\<alpha> f1 \<phi>1 = Pred\<^sub>\<alpha> f2 \<phi>2"
    by simp
qed

lemma Act\<^sub>\<alpha>_eq_iff: "Act\<^sub>\<alpha> f1 \<alpha>1 t1 = Act\<^sub>\<alpha> f2 \<alpha>2 t2 \<longleftrightarrow> tAct f1 \<alpha>1 (rep_Tree\<^sub>\<alpha> t1) =\<^sub>\<alpha> tAct f2 \<alpha>2 (rep_Tree\<^sub>\<alpha> t2)"
...
lemma Tree\<^sub>\<alpha>_free [simp]:
  ...
  and "Conj\<^sub>\<alpha> tset\<^sub>\<alpha> \<noteq> Pred\<^sub>\<alpha> f \<phi>"
  and "Conj\<^sub>\<alpha> tset\<^sub>\<alpha> \<noteq> Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha>"
  and "Not\<^sub>\<alpha> t\<^sub>\<alpha> \<noteq> Pred\<^sub>\<alpha> f \<phi>"
  and "Not\<^sub>\<alpha> t1\<^sub>\<alpha> \<noteq> Act\<^sub>\<alpha> f \<alpha> t2\<^sub>\<alpha>"
  and "Pred\<^sub>\<alpha> f1 \<phi> \<noteq> Act\<^sub>\<alpha> f2 \<alpha> t\<^sub>\<alpha>"
...
lemma supp_Pred\<^sub>\<alpha> [simp]: "supp (Pred\<^sub>\<alpha> f \<phi>) = supp f \<union> supp \<phi>"
unfolding supp_def by (simp add: Collect_imp_eq Collect_neg_eq)

lemma supp_Act\<^sub>\<alpha> [simp]:
  assumes "finite (supp t\<^sub>\<alpha>)"
  shows "supp (Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha>) = supp f \<union> (supp \<alpha> \<union> supp t\<^sub>\<alpha> - bn \<alpha>)"
using assms by (metis Act\<^sub>\<alpha>.abs_eq Tree\<^sub>\<alpha>_abs_rep Tree\<^sub>\<alpha>_rep_abs alpha_Tree_supp_rel supp_alpha_supp_rel supp_rel_tAct) (* Patience *)
...
lemma Tree\<^sub>\<alpha>_induct [case_names Conj\<^sub>\<alpha> Not\<^sub>\<alpha> Pred\<^sub>\<alpha> Act\<^sub>\<alpha> Env\<^sub>\<alpha>, induct type: Tree\<^sub>\<alpha>]:
  ...
  assumes "\<And>tset\<^sub>\<alpha>. (\<And>x. x \<in> set_bset tset\<^sub>\<alpha> \<Longrightarrow> P x) \<Longrightarrow> P (Conj\<^sub>\<alpha> tset\<^sub>\<alpha>)"
    ...
    and "\<And>f pred. P (Pred\<^sub>\<alpha> f pred)"
    and "\<And>f act t\<^sub>\<alpha>. P t\<^sub>\<alpha> \<Longrightarrow> P (Act\<^sub>\<alpha> f act t\<^sub>\<alpha>)"
  shows "P t\<^sub>\<alpha>"
...
inductive hereditarily_fs :: "('idx,'pred::fs,'act::bn,'eff::fs) Tree\<^sub>\<alpha> \<Rightarrow> bool" where
...
| Pred\<^sub>\<alpha>: "hereditarily_fs (Pred\<^sub>\<alpha> f \<phi>)"
| Act\<^sub>\<alpha>: "hereditarily_fs t\<^sub>\<alpha> \<Longrightarrow> hereditarily_fs (Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha>)"
...
lemma hereditarily_fs_alpha_renaming:
  assumes "Act\<^sub>\<alpha> f \<alpha> t\<^sub>\<alpha> = Act\<^sub>\<alpha> f' \<alpha>' t\<^sub>\<alpha>'"
...
typedef ('idx,'pred::fs,'act::bn,'eff::fs) formula = "{t\<^sub>\<alpha>::('idx,'pred,'act,'eff) Tree\<^sub>\<alpha>. hereditarily_fs t\<^sub>\<alpha>}"
...
begin
  ...
  lift_definition permute_formula :: "perm \<Rightarrow> ('a,'b,'c,'d) formula \<Rightarrow> ('a,'b,'c,'d) formula"
    is permute
...
lift_definition Not :: "('idx,'pred,'act,'eff) formula \<Rightarrow> ('idx,'pred::fs,'act::bn,'eff::fs) formula" is
  Not\<^sub>\<alpha>
by (fact hereditarily_fs.Not\<^sub>\<alpha>)

lift_definition Pred :: "'eff \<Rightarrow> 'pred \<Rightarrow> ('idx,'pred::fs,'act::bn,'eff::fs) formula" is
  Pred\<^sub>\<alpha>
by (fact hereditarily_fs.Pred\<^sub>\<alpha>)

lift_definition Act :: "'eff \<Rightarrow> 'act \<Rightarrow> ('idx,'pred,'act,'eff) formula \<Rightarrow> ('idx,'pred::fs,'act::bn,'eff::fs) formula" is
  Act\<^sub>\<alpha>
...
lemma Pred_eqvt [eqvt, simp]: "p \<bullet> Pred f \<phi> = Pred (p \<bullet> f) (p \<bullet> \<phi>)"
by transfer simp

lemma Act_eqvt [eqvt, simp]: "p \<bullet> Act f \<alpha> x = Act (p \<bullet> f) (p \<bullet> \<alpha>) (p \<bullet> x)"
by transfer simp
...
lemma supp_Pred [simp]: "supp (Pred f \<phi>) = supp f \<union> supp \<phi>"
by (metis Pred.rep_eq supp_Pred\<^sub>\<alpha> supp_Rep_formula)

lemma supp_Act [simp]: "supp (Act f \<alpha> x) = supp f \<union> (supp \<alpha> \<union> supp x - bn \<alpha>)"
...
lemma Pred_eq_iff [simp]: "Pred f1 \<phi>1 = Pred f2 \<phi>2 \<longleftrightarrow> f1 = f2 \<and> \<phi>1 = \<phi>2"
by (metis Pred.rep_eq Pred\<^sub>\<alpha>_eq_iff)

lemma Act_eq_iff: "Act f1 \<alpha>1 x1 = Act f2 \<alpha>2 x2 \<longleftrightarrow> Act\<^sub>\<alpha> f1 \<alpha>1 (Rep_formula x1) = Act\<^sub>\<alpha> f2 \<alpha>2 (Rep_formula x2)"
by (metis Act.rep_eq Rep_formula_inverse)

text \<open>Helpful lemmas for dealing with equalities involving~@{const Act}.\<close>

lemma Act_eq_iff_perm: "Act f1 \<alpha>1 x1 = Act f2 \<alpha>2 x2 \<longleftrightarrow>
  f1 = f2 \<and> (\<exists>p. supp x1 - bn \<alpha>1 = supp x2 - bn \<alpha>2 \<and> (supp x1 - bn \<alpha>1) \<sharp>* p \<and> p \<bullet> x1 = x2 \<and> supp \<alpha>1 - bn \<alpha>1 = supp \<alpha>2 - bn \<alpha>2 \<and> (supp \<alpha>1 - bn \<alpha>1) \<sharp>* p \<and> p \<bullet> \<alpha>1 = \<alpha>2)"
...
lemma Act_eq_iff_perm_renaming: "Act f1 \<alpha>1 x1 = Act f2 \<alpha>2 x2 \<longleftrightarrow>
  f1 = f2 \<and> (\<exists>p. supp x1 - bn \<alpha>1 = supp x2 - bn \<alpha>2 \<and> (supp x1 - bn \<alpha>1) \<sharp>* p \<and> p \<bullet> x1 = x2 \<and> supp \<alpha>1 - bn \<alpha>1 = supp \<alpha>2 - bn \<alpha>2 \<and> (supp \<alpha>1 - bn \<alpha>1) \<sharp>* p \<and> p \<bullet> \<alpha>1 = \<alpha>2 \<and> supp p \<subseteq> bn \<alpha>1 \<union> p \<bullet> bn \<alpha>1)"
  (is "?l \<longleftrightarrow> ?r")
proof
...
lemma Tree_free [simp]:
  ...
  and "finite (supp xset) \<Longrightarrow> Conj xset \<noteq> Pred f \<phi>"
  and "finite (supp xset) \<Longrightarrow> Conj xset \<noteq> Act f \<alpha> x"
  and "Not x \<noteq> Pred f \<phi>"
  and "Not x1 \<noteq> Act f \<alpha> x2"
  and "Pred f1 \<phi> \<noteq> Act f2 \<alpha> x"
proof -
  show "finite (supp xset) \<Longrightarrow> Conj xset \<noteq> Not x"
    by (metis Conj_rep_eq Not.rep_eq Tree\<^sub>\<alpha>_free(1))
next
  show "finite (supp xset) \<Longrightarrow> Conj xset \<noteq> Pred f \<phi>"
    by (metis Conj_rep_eq Pred.rep_eq Tree\<^sub>\<alpha>_free(2))
next
  show "finite (supp xset) \<Longrightarrow> Conj xset \<noteq> Act f \<alpha> x"
    by (metis Conj_rep_eq Act.rep_eq Tree\<^sub>\<alpha>_free(3))
next
  show "Not x \<noteq> Pred f \<phi>"
    by (metis Not.rep_eq Pred.rep_eq Tree\<^sub>\<alpha>_free(4))
next
  show "Not x1 \<noteq> Act f \<alpha> x2"
    by (metis Not.rep_eq Act.rep_eq Tree\<^sub>\<alpha>_free(5))
next
  show "Pred f1 \<phi> \<noteq> Act f2 \<alpha> x"
...
inductive is_FL_formula :: "'effect first \<Rightarrow> ('idx,'pred,'act,'effect) formula \<Rightarrow> bool"
where
...
lemma is_FL_formula_eqvt [eqvt]: "x \<in> \<A>[F] \<Longrightarrow> p \<bullet> x \<in> \<A>[p \<bullet> F]"
proof (erule is_FL_formula.induct)
  fix xset :: "('a, 'pred, 'act, 'effect) formula set['a]" and F
  assume 1: "finite (supp xset)" and 2: "\<And>x. x \<in> set_bset xset \<Longrightarrow> p \<bullet> x \<in> \<A>[p \<bullet> F]"
  from 1 have "finite (supp (p \<bullet> xset))"
    by (metis permute_finite supp_eqvt)
  moreover from 2 have "\<And>x. x \<in> set_bset (p \<bullet> xset) \<Longrightarrow> x \<in> \<A>[p \<bullet> F]"
    by (metis (mono_tags) imageE permute_set_eq_image set_bset_eqvt)
  ultimately show "p \<bullet> Conj xset \<in> \<A>[p \<bullet> F]"
    using 1 by (simp add: Conj)
next
  fix F and x :: "('a, 'pred, 'act, 'effect) formula"
  assume "p \<bullet> x \<in> \<A>[p \<bullet> F]"
  then show "p \<bullet> Not x \<in> \<A>[p \<bullet> F]"
    by (simp add: Not)
next
  fix f and F :: "'effect first" and \<phi>
  assume "f \<in>\<^sub>f\<^sub>s F"
  then show "p \<bullet> Pred f \<phi> \<in> \<A>[p \<bullet> F]"
    by (simp add: Pred)
next
